...
本文档展示 EchoMind 的核心架构图。图片均已生成到 `wiki/assets/architecture/`，可直接在 wiki、README 或项目展示材料中引用。

当前架构重点体现：Docker Compose 一键部署、多 Agent 编排、RAG 知识库、三级记忆、动态 Skills、监控和端到端评测。

## 1. 总体架构

![EchoMind 总体架构](assets/architecture/01-overall-architecture.svg)

说明：

- 用户请求先进入 Nginx，再转发到 EchoMind 主服务。
...
![EchoMind /chat 主链路架构](assets/architecture/02-chat-flow.svg)

说明：

- `/chat` 会先读取 Redis 和 ChromaDB 中的上下文。
- 系统会通过 MCP 工具链执行查询改写、并行召回和结果重排。
- Orchestrator 根据意图选择 General、Technical 或 Billing Agent。
- Agent 调用 LLM 前会通过 SkillManager 注入匹配的动态 Skills。
- 回复完成后，系统写入工作记忆，并异步更新用户画像。

## 3. 多 Agent 与 Skills 注入关系

![EchoMind 多 Agent 与 Skills 注入关系](assets/architecture/03-agent-skills.svg)

说明：

- 用户消息先经过 IntentRecognizer，再交给 AgentOrchestrator。
...
![EchoMind 数据与存储架构](assets/architecture/04-data-storage.svg)
...
![EchoMind 监控与评测闭环](assets/architecture/05-monitor-eval.svg)
...
![EchoMind Docker Compose 一键部署结构](assets/architecture/06-deployment.svg)
